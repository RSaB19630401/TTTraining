...
async function hashPassword(password) {
  ...
  return arrayToBase64(salt) + ':' + arrayToBase64(hashArr);
}
...
async function verifyJWT(token, secret) {
  try {
    ...
    const sigBytes = base64ToArray(sigPadded);
...
async function ensureInitialData(env) {
  ...
  // Check if default verein exists
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      // Password change (allowed for all authenticated)
      ...
      // --- ASSESSMENTS ---
      if (path === 'assessments' && method === 'GET') return getAssessments(env, user, url);
...
async function handleLogin(request, env, verein) {
  ...
  const user = await env.DB.prepare(
    'SELECT * FROM users WHERE verein_id = ? AND LOWER(username) = LOWER(?)'
  ).bind(verein.id, username).first();
  ...
  // Block initial admin if another admin exists
  if (user.is_initial_admin) {
    const otherAdmin = await env.DB.prepare(
      'SELECT id FROM users WHERE verein_id = ? AND role = ? AND is_initial_admin = 0'
    ).bind(verein.id, 'admin').first();
...
async function getPlayers(env, user) {
  ...
  const rows = await env.DB.prepare('SELECT * FROM players WHERE verein_id = ? ORDER BY name, vorname').bind(user.verein_id).all();
  return json(rows.results.map(mapPlayer));
}

function mapPlayer(p) {
  return { ...p, mannschaft: safeJSON(p.mannschaft, []) };
}
...
async function savePlayer(request, env, user) {
  const check = requireRole(user, 'admin', 'trainer');
  ...
  await env.DB.prepare(
    `INSERT INTO players (id, verein_id, name, vorname, geburtsdatum, jahrgang, mannschaft, gruppe, hand, trainer, vh_name, vh_typ, rh_name, rh_typ)
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)`
  ...
  // Auto-create user account for Spieler
  const uname = await genUsername(env, user.verein_id, data.vorname, data.name);
  const hash = await hashPassword('TVB1912');
  await env.DB.prepare(
    `INSERT INTO users (id, verein_id, username, password_hash, role, display_name, player_id, must_change_password)
     VALUES (?,?,?,?,?,?,?,1)`
...
async function getAssessments(env, user, url) {
  ...
  if (user.role === 'spieler') {
    const rows = await env.DB.prepare('SELECT * FROM assessments WHERE player_id = ? AND verein_id = ? ORDER BY date DESC')
      .bind(user.player_id, user.verein_id).all();
    return json(rows.results.map(mapAssessment));
  }
  if (playerId) {
    const rows = await env.DB.prepare('SELECT * FROM assessments WHERE player_id = ? AND verein_id = ? ORDER BY date DESC')
      .bind(playerId, user.verein_id).all();
    return json(rows.results.map(mapAssessment));
  }
  const rows = await env.DB.prepare('SELECT * FROM assessments WHERE verein_id = ? ORDER BY date DESC').bind(user.verein_id).all();
  return json(rows.results.map(mapAssessment));
}
...
async function saveAssessment(request, env, user) {
  ...
  const id = uid();
  await env.DB.prepare(
    `INSERT INTO assessments (id, verein_id, player_id, date, trainer, ratings, staerken, next_step, bemerkung)
     VALUES (?,?,?,?,?,?,?,?,?)`
...
async function updateAssessment(request, env, user, assessmentId) {
  const check = requireRole(user, 'admin', 'trainer');
  if (check) return check;
  const data = await request.json();
  await env.DB.prepare(
    ...
    data.staerken || '', data.nextStep || '', data.bemerkung || '', assessmentId, user.verein_id).run();
  return json({ ok: true });
}

async function deleteAssessment(env, user, assessmentId) {
  const check = requireRole(user, 'admin', 'trainer');
  if (check) return check;
  await env.DB.prepare('DELETE FROM assessments WHERE id = ? AND verein_id = ?').bind(assessmentId, user.verein_id).run();
  return json({ ok: true });
}
...
async function getTrainers(env, user) {
  const rows = await env.DB.prepare('SELECT * FROM trainers WHERE verein_id = ? ORDER BY name, vorname').bind(user.verein_id).all();
...
async function saveTrainer(request, env, user) {
  ...
  await env.DB.prepare(
    `INSERT INTO users (id, verein_id, username, password_hash, role, display_name, trainer_id, must_change_password)
     VALUES (?,?,?,?,?,?,?,1)`
...
async function updateTrainer(request, env, user, trainerId) {
  const check = requireRole(user, 'admin');
  if (check) return check;
  const data = await request.json();
  await env.DB.prepare('UPDATE trainers SET name=?, vorname=?, role=? WHERE id=? AND verein_id=?')
    .bind(data.name, data.vorname, data.role || 'trainer', trainerId, user.verein_id).run();
  await env.DB.prepare('UPDATE users SET display_name=?, role=? WHERE trainer_id=? AND verein_id=?')
    .bind(`${data.vorname} ${data.name}`, data.role || 'trainer', trainerId, user.verein_id).run();
  return json({ ok: true });
}

async function deleteTrainer(env, user, trainerId) {
  const check = requireRole(user, 'admin');
  if (check) return check;
  await env.DB.prepare('DELETE FROM users WHERE trainer_id = ? AND verein_id = ?').bind(trainerId, user.verein_id).run();
  await env.DB.prepare('DELETE FROM trainers WHERE id = ? AND verein_id = ?').bind(trainerId, user.verein_id).run();
  return json({ ok: true });
}
...
async function getLists(env, user) {
  ...
  // Merge trainer names from trainers table
  const trainers = await env.DB.prepare('SELECT vorname, name FROM trainers WHERE verein_id = ?').bind(user.verein_id).all();
  trainers.results.forEach(t => {
    const fullName = `${t.vorname} ${t.name}`;
    if (!result.trainerNamen.includes(fullName)) result.trainerNamen.push(fullName);
  });
...
async function addListItem(request, env, user) {
  ...
  try {
    ...
    if (e.message.includes('UNIQUE')) return err('Eintrag existiert bereits.');
    throw e;
  }
  return json({ ok: true });
}
...
async function getUsers(env, user) {
  const check = requireRole(user, 'admin');
  if (check) return check;
  const rows = await env.DB.prepare('SELECT id, username, role, display_name, player_id, trainer_id, is_initial_admin, must_change_password FROM users WHERE verein_id = ? AND is_initial_admin = 0 ORDER BY display_name').bind(user.verein_id).all();
  return json(rows.results.map(u => ({ ...u, is_initial_admin: !!u.is_initial_admin, must_change_password: !!u.must_change_password })));
}

async function resetPassword(env, user, targetUserId) {
  const check = requireRole(user, 'admin');
  if (check) return check;
  const target = await env.DB.prepare('SELECT * FROM users WHERE id = ? AND verein_id = ?').bind(targetUserId, user.verein_id).first();
  if (!target) return err('Benutzer nicht gefunden.');
  const pw = target.role === 'spieler' ? 'TVB1912' : 'TVB1912admin';
  const hash = await hashPassword(pw);
  await env.DB.prepare('UPDATE users SET password_hash = ?, must_change_password = 1 WHERE id = ?').bind(hash, targetUserId).run();
  return json({ ok: true });
}
...
async function exportData(env, user) {
  ...
  const lists = {};
  const listRows = (await env.DB.prepare('SELECT list_key, value FROM lists WHERE verein_id = ?').bind(user.verein_id).all()).results;
  listRows.forEach(r => { if (!lists[r.list_key]) lists[r.list_key] = []; lists[r.list_key].push(r.value); });
  ...
  if (data.players) {
    for (const p of data.players) {
      ...
      if (existing) {
        ...
        // Create user for imported player
    ...
    for (const [key, values] of Object.entries(data.lists)) {
      if (Array.isArray(values)) {
        for (const v of values) {
          try {
            await env.DB.prepare('INSERT INTO lists (verein_id, list_key, value) VALUES (?,?,?)').bind(vid, key, v).run();
