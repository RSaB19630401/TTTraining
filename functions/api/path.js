...
export async function onRequest(context) {
  ...
  try {
    ...
    // --- VIDEOS ---
    if (path === 'videos' && method === 'GET') return getVideos(env, user);
    if (path === 'videos' && method === 'POST') return saveVideo(request, env, user);
    if (path.startsWith('videos/') && method === 'PUT') return updateVideo(request, env, user, path.split('/')[1]);
    if (path.startsWith('videos/') && method === 'DELETE') return deleteVideo(env, user, path.split('/')[1]);
...
// VIDEOS
// ============================================================
async function getVideos(env, user) {
  const rows = await env.DB.prepare('SELECT * FROM videos WHERE verein_id=? ORDER BY category, sort_order, title').bind(user.verein_id).all();
  return json(rows.results.map(v => ({ ...v, skill_ids: safeJSON(v.skill_ids, []) })));
}

async function saveVideo(request, env, user) {
  const check = requireRole(user, 'admin', 'trainer'); if (check) return check;
  const data = await request.json();
  if (!data.title || !data.url) return err('Titel und URL sind Pflicht.');
  const id = uid();
  await env.DB.prepare(
    'INSERT INTO videos (id,verein_id,title,description,url,skill_ids,category,sort_order,created_by) VALUES (?,?,?,?,?,?,?,?,?)'
  ).bind(id, user.verein_id, data.title, data.description||'', data.url,
    JSON.stringify(data.skill_ids||[]), data.category||'Allgemein', data.sort_order||0, user.display_name).run();
  return json({ ok: true, id });
}

async function updateVideo(request, env, user, videoId) {
  const check = requireRole(user, 'admin', 'trainer'); if (check) return check;
  const data = await request.json();
  if (!data.title || !data.url) return err('Titel und URL sind Pflicht.');
  await env.DB.prepare(
    'UPDATE videos SET title=?,description=?,url=?,skill_ids=?,category=?,sort_order=? WHERE id=? AND verein_id=?'
  ).bind(data.title, data.description||'', data.url,
    JSON.stringify(data.skill_ids||[]), data.category||'Allgemein', data.sort_order||0, videoId, user.verein_id).run();
  return json({ ok: true });
}

async function deleteVideo(env, user, videoId) {
  const check = requireRole(user, 'admin', 'trainer'); if (check) return check;
  await env.DB.prepare('DELETE FROM videos WHERE id=? AND verein_id=?').bind(videoId, user.verein_id).run();
  return json({ ok: true });
}
